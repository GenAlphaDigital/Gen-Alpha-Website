...
const About = () => {
  ...
  useEffect(() => {
    // Ensure this code only runs on the client (in the browser)
    if (typeof window !== "undefined") {
      ...
      const observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            ...
            if (entry.isIntersecting && boxIndex !== -1) {
              // Set the correct gif based on the box in view
              setCurrentGif(gifs[boxIndex]);
              setDirection(1); // Play forward when entering
            } else if (entry.boundingClientRect.top > 0 && boxIndex !== -1) {
              // Reverse the gif when scrolling up
              setDirection(-1); // Play reverse when leaving upwards
            }
          });
        },
      ...
      // Observe each box once refs are ready
      boxRefs.forEach((ref) => {
        if (ref.current) {
          ...
          <div className={styles.box2}>
            ...
            >
              ...
              <p>
                ...
                a standard and constantly pushes boundaries.
              </p>
            </div>
          </div>
        </div>
